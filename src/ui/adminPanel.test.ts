...
describe('F8 admin panel — governance & DOM', () => {
  ...
  it('3. only live modifiers are editable (prepared/legacy are not)', () => {
    ...
    expect(isEditablePath('combat.unitSpeed')).toBe(false);
  ...
  it('6. editing a live slider creates a correct, clamped override', () => {
    ...
    expect(Number.isNaN(setOverrideValue('yellow', 'combat.unitSpeed', 2))).toBe(true);
    expect(hasOverride('yellow', 'combat.unitSpeed')).toBe(false);
  });
  ...
  it('10. import rejects prepared & legacy-backed paths (keeps only live)', () => {
    const malicious = JSON.stringify({
      ...
      factionModifierOverrides: {
        yellow: {
          ...
          combat: { unitSpeed: 9 },                     // legacy-backed → rejected
    ...
    expect(hasOverride('yellow', 'combat.unitSpeed' as string)).toBe(false);
  ...
  it('11. import reports warnings for every rejected path', () => {
    const { warnings } = sanitizeOverrides({
      yellow: { combat: { unitSpeed: 9 }, special: { colonyAuraStrength: 5 } },
