...
export interface FactionCombatModifiers {
  ...
  /** Infantry-only movement-speed multiplier (higher = faster). Vehicles are
   *  unaffected (mirrors legacy factions.json infantrySpeed). */
...
export const FACTION_MODIFIERS: Record<FactionId, FactionModifiers> = {
  ...
  red: {
    ...
    combat: { infantryDamage: 1.0, vehicleDamage: 1.15, energyWeaponDamage: 1.0, infantrySpeed: 1.0, unitHull: 1.0, vehicleHull: 1.0, infantryHull: 1.0 },
  ...
  blue: {
    ...
    combat: { infantryDamage: 0.98, vehicleDamage: 1.0, energyWeaponDamage: 1.0, infantrySpeed: 1.0, unitHull: 1.15, vehicleHull: 1.0, infantryHull: 1.0 },
  ...
  green: {
    ...
    combat: { infantryDamage: 1.0, vehicleDamage: 1.0, energyWeaponDamage: 1.0, infantrySpeed: 1.15, unitHull: 0.90, vehicleHull: 1.0, infantryHull: 1.0 },
  ...
  yellow: {
    ...
    combat: { infantryDamage: 0.98, vehicleDamage: 1.0, energyWeaponDamage: 1.2, infantrySpeed: 1.0, unitHull: 1.0, vehicleHull: 1.0, infantryHull: 1.0 },
...
export function getModifiedInfantrySpeed(baseSpeed: number, factionId: FactionId): number {
  return baseSpeed * getFactionModifiers(factionId).combat.infantrySpeed;
}
...
export const MODIFIER_RUNTIME_METADATA: ModifierRuntimeMetadata[] = [
  ...
  // ---- MIGRATED in Phase 4b.2b → now LIVE via FACTION_MODIFIERS (admin-editable) ----
  { path: 'combat.infantrySpeed', status: 'live', runtimeSource: 'FACTION_MODIFIERS', adminEditable: true, migrationNeeded: false, description: 'Infanterie-Tempo × (nur Infanterie; Fahrzeuge unberührt). Migriert: resolveUnit liest getModifiedInfantrySpeed (spiegelt factions.json infantrySpeed).' },

  // ---- DEPRECATED alias (read-only marker; kept so import-governance still rejects non-live paths) ----
  { path: 'combat.unitSpeed', status: 'legacy_backed', runtimeSource: 'not_yet_integrated', adminEditable: false, migrationNeeded: false, description: 'DEPRECATED → combat.infantrySpeed. Falscher (zu breiter) Name; KEIN Runtime-Effekt mehr (Phase 4b.2b).' },
...
export function calculateFactionPowerScore(factionId: FactionId): FactionPowerScore {
  ...
  const attackPower = avg(m.combat.infantryDamage, m.combat.vehicleDamage, m.combat.energyWeaponDamage, m.combat.infantrySpeed);
