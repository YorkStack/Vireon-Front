...
- **Modelle:** `src/render/vehicleModels.ts` — Chassis (tracked/wheeled/hover/**monoWheel v2**/**walker**) × Rollen-Kit (hopper/crane/turret/AA-pods/dish …); `makeEntityGroup(..., visual)` cached per `unit:<faction>:<classId>`. Alle Hüllen/Türme nutzen **RoundedBoxGeometry** (`rbox`, gefaste Kanten statt Voxel-Look). MonoWheel = ein dominanter Panzerring, Gyro-Kabine IM Rad, Seitenarme, Sattel-Joch als Kit-Deck — **keine Stützräder** (User-Skizze). Walker = 4/6 Beine mit Knie-Gelenken + Fußplatten (`legCount` in ChassisSpec); Schritt-Wippen via `stepBounce` in `animateUnit` (nur bei Bewegung). Yellow: heavyTank=6-Bein, antiAir/support=4-Bein, Rest Einrad.
